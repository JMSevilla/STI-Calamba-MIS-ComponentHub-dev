...
import { BaseEmailForgotPassword, BaseFPSecurityAndPassword, EmailForgotPasswordInfer, FPSecurityAndPassword } from '../core/schema/forgot-password'
import { useAtom } from 'jotai'
import { EmailFPAtom, FPSecurityAndPasswordAtom } from '../core/atoms/forgot-password-atom'
...
import { OtpAtom } from '../core/atoms/account-setup-atom'
import { ControlledOtpField } from '../components/TextField/OtpField'
import { useNavigate } from 'react-router-dom'
const ForgotPassword: React.FC = () => {
    const navigate = useNavigate()
    const [emailfp, setEmailFp] = useAtom(EmailFPAtom)
    const [security, setSecurity] = useAtom(FPSecurityAndPasswordAtom)
    const [otp, setOtp] = useAtom(OtpAtom)
    const form = useForm<EmailForgotPasswordInfer>({
        mode: 'all',
        resolver: zodResolver(BaseEmailForgotPassword),
        defaultValues: emailfp
    })
    const formOtp = useForm<OtpType>({
        mode: 'all',
        resolver: zodResolver(OtpBaseSchema),
        defaultValues: otp
    })
    const formSecurityAndPassword = useForm<FPSecurityAndPassword>({
        mode: 'all',
        resolver: zodResolver(BaseFPSecurityAndPassword),
        defaultValues: security
    })
    const [steps, setSteps] = useState(0)
    const { ToastMessage } = useToastMessage()
    const { loading, setLoading } = useLoaders()
    const apiforgotpasswordEmail = useApiCallback(
        async (api, args: { email: string }) => 
        await api.auth.fpRequestEmail(args)
    )
    const apiforgotpasswordOtp = useApiCallback(
        async (api, args: { email: string | undefined, code: number}) =>
        await api.auth.fpOtpEntry(args)
    )
    const apichangepasswordFp = useApiCallback(
        async (api, args: { email: string | undefined, password: string }) =>
        await api.auth.changePasswordFP(args)
    )
    const { 
    ...
    function handleContinue() {
        handleSubmit((values) => {
            ...
            setLoading(!loading)
            apiforgotpasswordEmail.execute(obj)
            .then((res: AxiosResponse | undefined) => {
                if(res?.data === 200) {
                    setLoading(false)
                    ToastMessage(
                        "Successfully Sent an OTP to your email",
                        "top-right",
                        false,
                        true,
                        true,
    ...
    function handleContinueOTP(){
        formOtp.handleSubmit((values) => {
            const obj = {
                email: emailfp?.email,
                code: parseInt(values.code)
            }
            setLoading(true)
            apiforgotpasswordOtp.execute(obj)
            .then((res : AxiosResponse | undefined) => {
                if(res?.data === 200) {
                    setLoading(false)
                    ToastMessage(
                        "OTP has been validated successfully",
                        "top-right",
                        false,
                        true,
                        true,
    ...
    function handleContinueResetPassword(){
        formSecurityAndPassword.handleSubmit((values) => {
            setLoading(true)
            const obj = {
                email: emailfp?.email,
                password: values.newPassword
            }
            apichangepasswordFp.execute(obj)
            .then((res : AxiosResponse | undefined) => {
                if(res?.data === 200) {
                    setLoading(false)
                    ToastMessage(
        ...
        <>
            <div style={{
                    ...
                    <div className="py-2 px-26 text-center">
                        ...
                        Forgot password is a feature that allows users to reset their forgotten passwords. It typically involves verifying their identity through email or security questions and then creating a new password to regain access to their accounts.
              </p>
                    ...
                    <div className="w-full border-stroke dark:border-strokedark xl:w-1/2 xl:border-l-2">
                        ...
                        <div className="mb-4">
                            <div className="relative">
                            {
                                ...
                                : steps === 2 && 
                                <FormProvider {...formSecurityAndPassword}>
                                    <ControlledTextField 
                                                control={formSecurityAndPassword.control}
                                                name='newPassword'
                                                required
                                                shouldUnregister
                            ...
                            <button
                                disabled={
                                    steps === 0 ? !isValid : steps === 1
                                    ? !formOtp.formState.isValid : steps === 2 ?
                                    !formSecurityAndPassword.formState.isValid : !isValid
                                }
                                onClick={() => {
                                    if(steps === 0){
                                        handleContinue()
                                        ...
                                        handleContinueResetPassword()
                                    }
                                }}
                                style={{
                                    cursor: steps === 0 ?
                                    !isValid ? 'not-allowed' : 'pointer' : steps === 1
                                    ? !formOtp.formState.isValid ? 'not-allowed' : 'pointer'
                                    : steps === 2 ? !formSecurityAndPassword.formState.isValid ? 'not-allowed' : 'pointer' : 'pointer',
                                    marginTop: '15px'
                                }}
