...
import {
  useAdaptiveTicketId,
  useAdaptiveWithPushNotification,
  useReferences,
...
const TicketDetails: React.FC = () => {
  ...
  const { stringToColor, stringAvatarColumns, stringAvatarTicketDetails } =
  ...
  const apiFilteredWithNotifs = useApiCallback(
    async (api, id: string) => await api.internal.filteredTicketsNotifs(id)
  );
  const apiFindComlab = useApiCallback(
    async (api, id: string) => await api.internal.filteredComlabs(id)
  );
  ...
  const useMutateAssignToMe = () => {
    return useMutation((data: { ticketId: string; id: number | undefined }) =>
      apiAssignToMe.execute(data)
    );
  };
  const useMutateChangeStatus = useMutation(
    (data: { ticketId: string; status: number }) =>
  ...
  function getFilteredComlabs() {
    data?.length > 0 &&
      data.map((item: any) => {
        apiFindComlab.execute(item.comLab).then((res: any) => {
          setComLabs(res.data);
        });
      });
  }
  ...
  function getCurrentUserByAssignedTicket() {
    data?.length > 0 &&
      data.map((user: any) => {
        apiFindAccountById
          .execute(user.specificAssignee)
          .then((res: AxiosResponse | undefined) => {
            res?.data?.length > 0 &&
              res?.data.map((currAccs: any) => {
  ...
  const handleDeleteConfirmation = async () => {
    setLoading(!loading);
    await apiDeleteTicketById.execute(ticketId).then(async (res) => {
      if (res.data) {
        setLoading(false);
        ToastMessage(
        ...
        const findRoute: any = routes.find(
          (item) =>
            item.access === references?.access_level &&
            item.path.includes("/dashboard/admin/ticketing")
        )?.path;
        navigate(findRoute);
              ...
              <Grid container spacing={2}>
                      ...
                      <Chip
                        ...
                        color={
                          item.issueStatuses == 0
                            ? "success"
                            : item.issueStatuses == 1
                            ? "primary"
                            : item.issueStatuses == 2
                            ? "success"
                            : "default"
                        }
                ...
                <Grid item xs={4}>
                  <BaseCard>
                    {references?.access_level === 1 && (
                      <>
                        ...
                        <div style={{ display: "flex", marginTop: "10px" }}>
                          <BasicSelectField
                            label="Status"
                            disabled={item.specificAssignee == 0 ? true : false}
                            options={[
                              {
                                value: 0,
                                label: "Open",
                    ...
                    <hr />
                    <br />
                    <Typography sx={{ mr: 1 }} gutterBottom variant="caption">
                      <span
                        style={{
                          color: "darkgray",
                        }}
                    ...
                    >
                      {item.accounts.imgurl === "no-image" ? (
                        <Avatar
                          {...stringAvatarTicketDetails(
                            item.accounts.firstname +
                              " " +
                              item.accounts.lastname
                          )}
                        />
                      ) : (
                        <Avatar
                    ...
                    >
                      ...
                      <Chip
                        ...
                        label={item.priority}
                        color={
                          item.priority == "lowest" || item.priority == "low"
                            ? "primary"
                    ...
                    >
                      {JSON.parse(item.pc_number)?.length > 0 &&
                        JSON.parse(item.pc_number)?.map((pc: any) => (
                          <>
                            <Chip
                              size="small"
                              sx={{ mr: 1 }}
                    ...
                    >
                      {JSON.parse(item.issue)?.length > 0 &&
                        JSON.parse(item.issue)?.map((is: any) => (
                          <>
                            <Chip
                              size="small"
                              sx={{ mr: 1 }}
