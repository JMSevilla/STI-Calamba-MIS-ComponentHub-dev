...
import {
  AccountModeratorProps,
  AccountSetupProps,
  AccountStudentProps,
  SubjectStartAssignation,
} from "../../core/types";
...
import {
  ...
  MultipleSelectField,
} from "../../components/SelectField/MultiSelectField";
...
const AddNewStudentForm = () => {
  const { control, getValues, watch, trigger, resetField, setValue } =
    ...
    <div className="p-6.5">
          ...
          <>
            <BasicSelectField
              ...
              options={[
                ...
                {
                  value: "@calamba.sti.edu.ph",
                  label: "@calamba.sti.edu.ph",
                },
              ]}
      ...
      <div className="mb-4.5">
        ...
        <div style={{ display: "flex" }}>
          ...
          <Button onClick={GenPass} size="small" sx={{ ml: 1 }}>
            Generate Password
          </Button>
        </div>
      </div>
...
const AddNewStudent = () => {
  ...
  const apiAssignedSubjectsByCourse = useApiCallback(
    async (api, args: { courseId: number; accountId: number[] }) =>
  ...
  async function startSubjectAssignation() {
    setLoading(true);
    const selectedItems = left.filter((item: any) =>
      leftChecked.includes(item)
    );
    for (const subjectLeft of selectedItems) {
      for (const ac of accounts) {
        ...
        await apiStartSubjectAssignation.execute(obj).then((res) => {
          if (res.data === 400) {
            setLoading(false);
            ToastMessage(
  ...
  function startDismantlingSubjectsToStudents() {
    setLoading(true);
    const selectedItems = right.filter((item: any) =>
      rightChecked.includes(item)
    );
    console.log(selectedItems);
    selectedItems?.length > 0 &&
      selectedItems.map((dismantle: any) => {
        apiDismantleSubjectToStudents.execute(dismantle.id).then((res: any) => {
          if (res.data === 200) {
            setLoading(false);
            ToastMessage(
  ...
  const { preload, setPreLoad, loading, setLoading, gridLoad, setGridLoad } =
    useLoaders();
  const { ToastMessage } = useToastMessage();
  const apiCourseList = useApiCallback((api) =>
    api.internal.getAllCoursesNonJoined()
  );
  ...
  const apiSectionList = useApiCallback(
    async (api, course_id: number) =>
      await api.internal.getAllSectionsNonJoined(course_id)
  );
  const apiResendOtp = useApiCallback(
    async (api, args: { email: string | undefined }) =>
      await api.internal.accountResendOtp(args)
  );
  ...
  const apiCourseManagementList = useApiCallback((api) =>
    api.internal.coursemanagementList()
  );
  const apiArchiveAccount = useApiCallback(
    async (api, accountId: number) =>
      await api.internal.accountArchive(accountId)
  );
  ...
  function viewProfile(accountId: number) {
    const findRoute: any = routes.find(
      (route) =>
        route.access === references?.access_level &&
        route.path.includes("/dashboard/moderator/profile-details")
    )?.path;
            ...
            return (
              <div style={{ display: "flex" }}>
                {params.row.status === 1 ? (
                  <IconButton
                    onClick={() =>
                      triggerAccountDeletionToArchive(params.row.id)
                    }
                    color="error"
                    size="small"
                  >
                  ...
                  <IconButton
                    onClick={() => handleRecoverAccount(params.row.id)}
                    color="primary"
                    size="small"
                  >
                    <RotateLeftIcon />
                ...
                {params.row.verified === 0 && (
                  <IconButton
                    onClick={() => handleResendOtp(params.row.email)}
                    color="warning"
                    size="small"
                  >
                    <SendIcon />
                ...
                <Tooltip title="Archive Account">
                  <IconButton
                    onClick={() => ArchiveAccount(params.row.id)}
                    color="primary"
                    size="small"
                  >
                    <ArchiveIcon />
  ...
  function courseList() {
    apiCourseManagementList.execute().then((res) => {
  ...
  const useCreateStudent = () => {
    return useMutation(
      async (data: AccountModeratorProps) =>
  ...
  const handleSubmissionFromExcel = () => {
    ...
    csvData.length > 0 &&
      csvData.map(async (row: ExportData) => {
        if (
          row.firstname == undefined ||
          row.lastname == undefined ||
          row.email == undefined ||
          row.multipleSections == undefined ||
  ...
  function initializedSectionListFromSubjectAssignation(course_id: number) {
    apiSectionList.execute(course_id).then((res) => {
      const result =
        res.data?.length > 0 &&
  ...
  const handleSelectedCourse = (e: string) => {
    apiAccountsByCourse
      .execute({ courseId: parseInt(e), section_id: 0 })
      .then((repository) => {
  ...
  const handleSelectedSection = (e: string) => {
    apiAccountsByCourse
      .execute({ courseId: parseInt(selectedCourse), section_id: parseInt(e) })
      .then((repository) => {
        setStudents(repository.data);
  ...
  const memoizedSelectedStudentByCourse = useMemo(() => {
    function handleSelectedStudent(id: any) {
      ...
      apiSelectedSubjectByCourse.execute(obj).then((repository) => {
        setLoading(false);
        setLeft(repository.data);
        apiAssignedSubjectsByCourse.execute(obj).then((assignedRepository) => {
  ...
  function initializedSectionsByCourse() {
    ...
    if (values.course_id === undefined) {
      ...
      apiSectionList.execute(parseInt(values.course_id)).then((res) => {
        const result =
          res.data?.length > 0 &&
          res.data.map((item: any) => {
            return {
  ...
  const handleSelectedCsvCourse = (e: any) => {
    ...
    apiSectionList.execute(parseInt(e)).then((res) => {
      const result =
        res.data?.length > 0 &&
        res.data.map((item: any) => {
          return {
  ...
  const genEmptyExportArray = (count: number): ExportData[] => {
    const customExportArray: ExportData[] = Array.from(
      { length: count },
      (_, index) => ({
        id: index + 1,
        firstname: undefined,
        ...
        <>
          ...
          >
            <Typography variant="subtitle1" gutterBottom>
              This account may proceed to the archive data.
            </Typography>
          </ControlledModal>
          <ControlledModal
            open={csvDataPreview}
          ...
          >
            <Typography variant="button">Excel Data Preview</Typography>
