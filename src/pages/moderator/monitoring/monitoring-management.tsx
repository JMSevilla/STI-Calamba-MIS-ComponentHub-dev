...
import {
  ...
  BaseRoomPasswordSchema,
  CreateRoomInfer,
  RoomPasswordInfer,
} from "../../../core/schema/room";
import { useAtom } from "jotai";
import {
  CreateRoomAtom,
  RoomPasswordAtom,
} from "../../../core/atoms/room-atom";
...
const MonitoringManagement: React.FC = () => {
  ...
  const apiRevokeTheMeetingRoom = useApiCallback(
    async (api, room_id: string) => await api.internal.revokeRoom(room_id)
  );
  ...
  const apicreateroom = useApiCallback(
    async (api, args: CreateRoomInfer) => await api.internal.createNewRoom(args)
  );
  const apiGetAllRooms = useApiCallback(
    async (api, args: { section: number[] }) =>
      await api.internal.getAllRooms(args)
  );
  const apiJoinedParticipantsLogs = useApiCallback(
    async (api, args: MeetRoomJoinedProps) =>
      await api.internal.joinedParticipantsLogs(args)
  );
  ...
  const apiSubjectsList = useApiCallback(
    async (api, course_id: number) =>
      await api.internal.subjectsListByCourse(course_id)
  );
  ...
  const useCreateRoom = () => {
    return useMutation((data: CreateRoomInfer) => apicreateroom.execute(data));
  };
  ...
  function initializedSubjects() {
    ...
    if (values.sectionId !== undefined || values.sectionId?.length > 0) {
      apiSubjectsList
        .execute(values.sectionId?.length > 0 ? values.sectionId[0]?.value : 0)
        .then((res) => {
          const result =
            res.data?.length > 0 &&
  ...
  const values = getValues();
  const { preload, setPreLoad, gridLoad, setGridLoad } = useLoaders();
  useEffect(() => {
    setPreLoad(false);
    setLoading(false);
    TriggerComLabList();
  ...
  function ConfirmRevokeRoom() {
    setLoading(!loading);
    apiRevokeTheMeetingRoom.execute(revokeId).then((res) => {
      if (res.data === 200) {
  ...
  function TriggerComLabList() {
    apiComlabList.execute().then((res) => {
      const result =
        res.data?.length > 0 &&
        res.data?.map((item: any) => {
          return {
    ...
    handleSubmit(async (values) => {
      ...
      const findRoute: any = routes.find(
        (route) =>
          ...
          route.path.includes("/dashboard/moderator/meet")
      )?.path;
      setLoading(!loading);
      ...
      await mutateAsync(obj, {
        onSuccess: (response: AxiosResponse | undefined) => {
          if (response?.data?.status === 200) {
            ...
            apiCallJitsiServer.execute(jitsiObj).then((auth) => {
  ...
  function handleSubmitPrivateRoom() {
    formPassword.handleSubmit((values) => {
      ...
      setLoading(!loading);
      apiRoomPasswordIdentifier.execute(obj).then((res) => {
        if (res.data === 200) {
          initializedJoinRoom(
            roomDetails.room_name,
            obj.room_id,
            roomDetails.comlabId,
  ...
  function initializedJoinRoom(
    ...
    apiCallJitsiServer.execute(jitsiObj).then((auth) => {
      ...
      setTimeout(() => {
        reset();
        navigate(
          `${findRoute}?match=${concat}&room_id=${room_id}&type=${room_type}`
        );
      }, 2000);
      refetch();
    });
  ...
  const memoizedRoomList = useMemo(() => {
    ...
    function removeMeetingRoom(room_id: string) {
      apiRemoveRevokedRoom.execute(room_id).then((res) => {
        if (res.data === 200) {
          refetch();
        }
      });
    ...
    const columns: any = [
      ...
      },
      {
        ...
        valueGetter: (params: any) =>
          `${moment(params.row.created_at).calendar()}`,
      },
      {
        width: 220,
        renderCell: (params: any) => {
  ...
  function initializedSectionsByCourse() {
    apiSectionList.execute(references?.course).then((res) => {
      const result =
        res.data?.length > 0 &&
        res.data.map((item: any) => {
          return {
      ...
      {preload ? (
        ...
        <>
          ...
          <ControlledModal
            ...
            isValid={false}
          >
            <Typography variant="button">Room Revoke</Typography> <br />
