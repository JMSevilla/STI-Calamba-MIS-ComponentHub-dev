...
export const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  const [jwtAccessToken, setJwtAccessToken, clearJwtAccessToken] =
    useJitsiAccessToken();
  ...
  const [currentScreen, setCurrentScreen, clearCurrentScreen] =
    useCurrentScreen();
  const [warnedOrUnAuthParticipantModal, setWarnedOrUnAuthParticipantModal] =
    useWarningOrUnauthorizedModal();
  ...
  const apiRemoveFromTheCurrentMeeting = useApiCallback(
    async (api, id: number) => await api.internal.deleteJoinedParticipants(id)
  );
                        ...
                        {(handleClick, open) => {
                          return (
                            <>
                              ...
                              {item.customSubs?.length > 0 &&
                                item.customSubs?.map((customs) => (
                                  <div
                                      ...
                                      <li>
                                        <NavLink
                                          ...
                                          onClick={() => {
                                            ...
                                            clearCurrentScreen();
                                            setWarnedOrUnAuthParticipantModal(
                                              false
                                            );
                                            clearPt();
                                          }}
