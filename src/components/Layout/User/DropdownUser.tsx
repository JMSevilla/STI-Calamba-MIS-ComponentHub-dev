...
export const DropdownUser = () => {
    ...
    function selectedOption(){
      if(references?.access_level === 1) {
        const findRoute: any = routes.find((item) => item.access === references?.access_level && item.path.includes('/dashboard/admin/profile'))?.path
        navigate(findRoute)
        setDropdownOpen(false)
      } else if(references?.access_level === 2) {
        const findRoute: any = routes.find((item) => item.access === references?.access_level && item.path.includes('/dashboard/moderator/profile'))?.path
        navigate(findRoute)
        setDropdownOpen(false)
      } else {
        const findRoute: any = routes.find((item) => item.access === references?.access_level && item.path.includes('/dashboard/student/profile'))?.path
        navigate(findRoute)
        setDropdownOpen(false)
      }
    } 
        ...
        <div className='relative'>
            ...
            >
                ...
                <span className="block text-xs">
                    {references?.access_level == 1 ? 'Administrator' : references?.access_level == 2 ? `Professor` : references?.access_level == 3 && 'Student'}
                </span>
                </span>
