...
const DashboardLayout: React.FC = () => {
  ...
  const apicheckSignInRequest = useApiCallback(
    async (api, accountId: number) =>
      await api.auth._checkSigninRequest(accountId)
  );
  ...
  const apiWatchDisabledAccount = useApiCallback(
    async (api, accountId: number) =>
      await api.internal.disableAccountWatcher(accountId)
  );
  const apiWatchAccountNotVerified = useApiCallback(
    async (api, accountId: number) =>
      await api.auth.detectAccountIsNotVerified(accountId)
  );
  const { ToastMessage } = useToastMessage();
  function trackSigninRequest() {
    apicheckSignInRequest.execute(references?.id).then((res) => {
  ...
  function watchDisabledAccount() {
    apiWatchDisabledAccount.execute(references?.id).then((res) => {
      if (res.data) {
        logout();
  ...
  function watchAccountNotVerified() {
    apiWatchAccountNotVerified.execute(references?.id).then((res) => {
      if (res.data) {
        logout();
          ...
          <main>
            <div className="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
              {expirationTime != null &&
                ...
                AlertTracker(
                  `You are idle. Token expires in: ${FormatExpiry(
                    expirationTime
                  )}`,
                  "error"
                )}
              ...
              >
                ...
                <Typography variant="caption">
                  You are trying to sign-in to another device. Once approved you
                  will automatically logout
                </Typography>
              </ControlledModal>
              <ControlledModal
                disableButton
